...
RBS_svnrev = {}
RBS_svnrev["Core.lua"] = select(3,string.find("$Revision: 488 $", ".* (.*) .*"))
...
function RaidBuffStatus:COMBAT_LOG_EVENT_UNFILTERED(event, timestamp, subevent, hideCaster, srcGUID, srcname, srcflags, dstGUID, dstname, dstflags, spellID, spellname, spellschool, extraspellID, extraspellname, extraspellschool, auratype)
	if not raid.israid and not raid.isparty then
		return
	end
